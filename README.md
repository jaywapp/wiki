# wiki

> **jaywapp (Junyoung Park)** 의 개인 위키입니다.
> 개발 경험, 기술 정리, 프로젝트 메모 등을 기록합니다.

---

## 개요

이 저장소는 개인 지식 베이스(Personal Knowledge Base)로, 다음 내용을 다룹니다.

- **기술 노트** — C#/.NET, TypeScript, Flutter, AI/LLM 관련 학습 기록
- **프로젝트 메모** — 진행 중인 프로젝트의 설계 결정 사항, 트러블슈팅 로그
- **개발 팁** — 반복적으로 필요한 패턴, 코드 스니펫, 설정 참고
- **참고 자료** — 유용한 링크, 도구, 라이브러리 목록

## 관련 링크

- GitHub: [@jaywapp](https://github.com/jaywapp)
- 프로젝트 목록: [Projects](https://github.com/jaywapp/Projects)
- 블로그: [jaywapp.tistory.com](https://jaywapp.tistory.com/)
